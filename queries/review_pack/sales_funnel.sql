...
wechat_metrics AS (SELECT period,channel,dimension_type,CONCAT(dimension_value,'×数据源未接入') dimension_value,'企微添加人数' metric,CAST(NULL AS DOUBLE) value,'data_source_missing' source_version FROM summary UNION ALL SELECT period,channel,dimension_type,CONCAT(dimension_value,'×数据源未接入'),'企微添加率',CAST(NULL AS DOUBLE),'data_source_missing' FROM summary),
metrics AS (SELECT period,channel,dimension_type,dimension_value,metric,value,'v2;event_ordered_nested_funnel' source_version FROM numeric_metrics UNION ALL SELECT period,channel,dimension_type,dimension_value,metric,value,source_version FROM wechat_metrics)
